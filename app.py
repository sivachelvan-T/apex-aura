...
def content_matches_extension(file_bytes: bytes, extension: str) -> bool:
    """Check the file's real content (magic bytes), not just its name.

    A file named payslip.png that is secretly an executable will fail
    this check even though the extension looked fine.
    """
    extension = extension.lower()
    if extension == "pdf":
        return file_bytes[:5] == b"%PDF-"
    if extension in {"jpg", "jpeg"}:
        return imghdr.what(None, h=file_bytes) == "jpeg"
    if extension == "png":
        return imghdr.what(None, h=file_bytes) == "png"
    return False
